...
final readonly class ProjectsClient
{
    ...
    public function create(CreateProject $project, ?IdempotencyKey $idempotencyKey = null): MutationResult
    {
        return $this->gateway->create($project->toArray(), $idempotencyKey);
    }

    public function update(ObjectKey $key, UpdateProject $project, ?IdempotencyKey $idempotencyKey = null): MutationResult
    {
        return $this->gateway->update($key, $project->toArray(), $idempotencyKey);
    }
     ...
     *
     * @param  list<CreateProject>  $records
     */
    public function createMany(array $records, bool $atomic = false, ?IdempotencyKey $idempotencyKey = null): BatchResult
    {
        return $this->gateway->createMany(
            array_map(static fn (CreateProject $record): array => $record->toArray(), $records),
            $atomic,
            $idempotencyKey,
        );
    }
