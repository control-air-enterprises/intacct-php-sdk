...
final readonly class LocationsClient
{
    ...
    public function create(CreateLocation $location, ?IdempotencyKey $idempotencyKey = null): MutationResult
    {
        return $this->gateway->create($location->toArray(), $idempotencyKey);
    }

    public function update(ObjectKey $key, UpdateLocation $location, ?IdempotencyKey $idempotencyKey = null): MutationResult
    {
        return $this->gateway->update($key, $location->toArray(), $idempotencyKey);
    }
     ...
     *
     * @param  list<CreateLocation>  $records
     */
    public function createMany(array $records, bool $atomic = false, ?IdempotencyKey $idempotencyKey = null): BatchResult
    {
        return $this->gateway->createMany(
            array_map(static fn (CreateLocation $record): array => $record->toArray(), $records),
            $atomic,
            $idempotencyKey,
        );
    }
